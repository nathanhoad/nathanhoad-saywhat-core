 ...
 * @param lines
 */
export function linesToText(lines: Array<INodeLine> | null): string {
  if (!lines) return "";
 ...
 * @param otherNodes
 */
export function responsesToText(responses: Array<INodeResponse> | null, otherNodes: Array<INode>): string {
  if (!responses) return "";

  const byId = keyBy("id", otherNodes);
  return responses
    .map(response => {
      // Find the node that this response points to (if there is one)
      const goToNode =
        response.goToNodeId && Object.keys(byId).includes(response.goToNodeId) ? byId[response.goToNodeId] : null;
      const name = goToNode?.name || response.goToNodeName;
