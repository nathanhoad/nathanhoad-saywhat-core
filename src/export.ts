  ...
  const xml = project.sequences
    .map(sequence => {
      ...
      const sequenceXml = sequence.nodes
        .map(node => {
          let entryPoint: string | null = node.id;
  ...
  const exportedSequences = keyBy(
    ...
    project.sequences.map(sequence => {
      ...
      return {
        ...
        nodes: keyBy(
          "id",
          sequence.nodes.reduce<Array<IExportedNode>>((nodes, node) => {
            let entryPoint: string | null = node.id;
...
export function projectToExportNodesList(project: IProject): Array<IGodotLine> {
  return project.sequences.reduce<Array<IGodotLine>>((list, sequence) => {
    const nodesById = keyBy("id", sequence.nodes);

    return list.concat(
      sequence.nodes.reduce<Array<IGodotLine>>((nodes, node) => {
        let entryPoint: string | null = node.id;
...
type INodeLineType = "comment" | "mutation" | "goto" | "dialogue" | "responses";
...
export function getType(line: INodeLine): INodeLineType {
  if (line.comment) return "comment";

  if (line.goToNodeId || line.goToNodeName) return "goto";
